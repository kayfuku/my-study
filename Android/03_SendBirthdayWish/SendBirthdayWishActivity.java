...
public class SendBirthdayWishActivity extends Activity 
       implements OnCheckedChangeListener
{
	...
	public void checkHappyGuy()
	{
	    ...
	    SimpleDateFormat formatterDayOfMonth = 
	    ...
	    Log.i("birth", "rawQuery() begin. " + date[0] + 
		      "/" + date[1]);
	    cursor = db.rawQuery(
	    		"select name, email, tel_number from " + 
				DB_TABLE + " " +
	    ...
	    messageChanged = message.replaceAll("xxx", 
	    		            happyGuys.get(indexGuy).getName());
	...
	public void send()
	{
		if (rbSMS.isChecked())
		{
			...
			{
				String address = 
					new String(happyGuys.get(indexGuy).getSMS());
		...
		else
		{
			...
			{
				String address = 
					new String(happyGuys.get(indexGuy).getEmail());
