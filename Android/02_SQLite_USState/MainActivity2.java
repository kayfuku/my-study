...
public class MainActivity2 extends Activity implements View.OnKeyListener
{
    ...
    public boolean onKey(View v, int keyCode, KeyEvent ke)
    {
                ...
                && (keyCode == KeyEvent.KEYCODE_ENTER))
        {
            ...
            {
                Toast t = Toast.makeText(getBaseContext(), "Enter your name.",
                        Toast.LENGTH_LONG);
                t.show();
            }
            else
    ...
    private void insert()
    {
        ...
        /*
         * // Create a DB_TABLE2 table. The first three lines // were worth
         * working for hours! sqlcmd = ""; sqlcmd += "drop table if exists " +
         * DB_TABLE2 + ";"; db.execSQL(sqlcmd); sqlcmd = ""; sqlcmd +=
         * "create table " + DB_TABLE2 + " ("; sqlcmd += DB_COLUMN_RANK +
         * " integer, "; sqlcmd += DB_COLUMN_NAME + " text not null, "; sqlcmd
         * += DB_COLUMN_SCORE + " integer"; sqlcmd += ")"; db.execSQL(sqlcmd);
         */
        ...
        for (int i = 0; i < 50; i++)
        {
            cv.put(DB_COLUMN_STATE, states[i]);
            cv.put(DB_COLUMN_CAPITAL, capitals[i]);
