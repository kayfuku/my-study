	...
	private void getResult()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			c = db.query(MainActivity2.DB_TABLE, 
					     new String[]{MainActivity2.DB_COLUMN_CAPITAL}, 
					     "state = ?",
					     new String[]{states[indexStatesSelected[i]]}, 
					     null, null, null, null);
			Log.i("USGame", "query.");
			c.moveToFirst();
		...
		{
			c = db.query(MainActivity2.DB_TABLE, 
				     new String[]{MainActivity2.DB_COLUMN_STATE}, 
				     "capital = ?",
				     new String[]{capitals[indexCapitalsSelected[i - 5]]}, 
				     null, null, null, null);
			c.moveToFirst();
	...
	private void pickUp(String[] strs, boolean statesFlag)
	{
		...
		while (i < 5)
		{
			...
			if (isInArray)
			{
				...
				{
					tvs[i].setText((i + 1) + ": " + 
						       strs[indexSelected[i]]);
					indexStatesSelected[i] = indexSelected[i];
				}
				else
				{
					tvs[i + 5].setText((i + 1 + 5) + ": " + 
						       strs[indexSelected[i]]);
