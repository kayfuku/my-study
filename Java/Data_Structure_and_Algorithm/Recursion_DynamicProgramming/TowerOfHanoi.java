...
public class TowerOfHanoi {
	...
	public static void main(String[] args) {
		...
		// Move all the disks from tower 0 to tower 2.
...
class Tower {
	...
	// d is the size of disk to be added. 
	public void addDisk(int d) {
		if (!disks.isEmpty() 
			&& disks.getFirst() < d /* Restriction of tower */) {
	...
	public void moveDisks(int n, Tower destination, Tower buffer) {
		...
		if (n > 0) {
			// Move n - 1 disks to the buffer tower using 
			// destination tower as a buffer. 
			moveDisks(n - 1, buffer, destination);
			// Move n-th disk to the destination.
			moveTopTo(destination);
			// Move n - 1 disks from buffer tower to the destination. 
			// 'this' is a tower that is calling moveDisks(). 
